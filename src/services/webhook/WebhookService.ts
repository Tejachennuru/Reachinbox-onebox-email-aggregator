...
export class WebhookService {
  async triggerInterestedWebhooks(email: EmailDocument): Promise<void> {
    console.log(`🔔 Triggering webhooks for Interested email: ${email.subject}`);
  ...
  private async sendGenericWebhook(email: EmailDocument): Promise<void> {
    ...
    const sanitizedEmail: any = {
      ...
      ...(Array.isArray(email.cc) && email.cc.length > 0 ? { cc: email.cc.slice(0, 5) } : {}),
      date: email.date,
    ...
    if (Array.isArray(config.webhooks.interestedUrls)) {
      for (const u of config.webhooks.interestedUrls.filter(Boolean)) urlsSet.add(u);
    }
    ...
    if (urls.length > 0) {
      const results = await Promise.all(urls.map((u) => postSafely(u, basePayload)));
      const successCount = results.filter(Boolean).length;
      console.log(`✅ Generic/Interested webhooks triggered (${successCount}/${urls.length})`);
    }
  }
